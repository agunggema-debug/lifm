...
export function playMatchdayFirstHalf(save) {
  const clubs = clubMap();
  const fixtures = db.prepare('SELECT * FROM fixtures WHERE season=? AND matchday=? ORDER BY id').all(save.season, save.matchday);
  if (!fixtures.length) {
    const champ = db.prepare('SELECT c.* FROM standings_cache s JOIN clubs c ON c.id=s.club_id ORDER BY s.points DESC, s.gd DESC, s.gf DESC').get();
  ...
  for (const f of fixtures) {
    ...
    if (isUser) {
      ...
      const mine = xiFor(save.club_id, lineupIds, save.formation, save.mentality);
      const opp = autoXI(isHome ? f.away_id : f.home_id, '4-4-2', 'balanced').xi;
      ...
      homeXI = autoXI(f.home_id, '4-4-2', 'balanced').xi;
      awayXI = autoXI(f.away_id, '4-4-2', 'balanced').xi;
...
export function playMatchdaySecondHalf(save, body) {
  const clubs = clubMap();
  const halfTimeState = body.halfTimeState || [];
  if (!halfTimeState.length) return playMatchdayFirstHalf(save);
  let userResult = null;
  const others = [];
  for (const hs of halfTimeState) {
    const f = db.prepare('SELECT * FROM fixtures WHERE id=?').get(hs.fixtureId);
    if (!f) continue;
    ...
    if (isUser) {
      const isHome = f.home_id === save.club_id;
      const myT = { formation: save.formation, mentality: save.mentality };
      const mine2 = xiFor(save.club_id, lineupIds2, save.formation, save.mentality);
      const opp = autoXI(isHome ? f.away_id : f.home_id, '4-4-2', 'balanced').xi;
      ...
      homeXI2 = autoXI(f.home_id, '4-4-2', 'balanced').xi;
      awayXI2 = autoXI(f.away_id, '4-4-2', 'balanced').xi;
      homeT = { formation: '4-4-2', mentality: 'balanced' };
      awayT = { formation: '4-4-2', mentality: 'balanced' };
    }
    ...
    if (isUser) {
      ...
      const s1 = oppAutoSub(oppArr, oppId, oppSide, oppShort, 58 + Math.floor(Math.random() * 5));
      if (s1) res.events.push(s1);
      const s2 = oppAutoSub(oppArr, oppId, oppSide, oppShort, 71 + Math.floor(Math.random() * 6));
      if (s2) res.events.push(s2);
      res.events.sort(function (a, b) { return a.minute - b.minute; });
    }
    db.prepare('UPDATE fixtures SET played=1, home_goals=?, away_goals=?, events_json=? WHERE id=?')
      .run(res.homeGoals, res.awayGoals, JSON.stringify(isUser ? res.events : []), f.id);
    bump(f.home_id, res.homeGoals, res.awayGoals);
    bump(f.away_id, res.awayGoals, res.homeGoals);
    pm2(homeXI2, 'home', res);
    pm2(awayXI2, 'away', res);
  ...
  db.prepare('UPDATE players SET injured_weeks = injured_weeks - 1 WHERE injured_weeks > 0').run();
  db.prepare('INSERT INTO news (day_label,title,body,tag) VALUES (?,?,?,?)').run(
    'MD' + save.matchday,
  ...
  db.prepare("UPDATE saves SET matchday=?, updated_at=datetime('now') WHERE id=1").run(next);
  return { userResult: userResult, others: others, nextMatchday: next, finished: next > 17 };
}

function bump(clubId, gf, ga) {
  const won = gf > ga ? 1 : 0;
  const drawn = gf === ga ? 1 : 0;
  const lost = gf < ga ? 1 : 0;
  const pts = won ? 3 : drawn ? 1 : 0;
  db.prepare('UPDATE standings_cache SET played=played+1, won=won+?, drawn=drawn+?, lost=lost+?, gf=gf+?, ga=ga+?, gd=gd+?, points=points+? WHERE club_id=?')
    .run(won, drawn, lost, gf, ga, gf - ga, pts, clubId);
}
