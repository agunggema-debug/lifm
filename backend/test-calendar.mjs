...
must('elite-128-laga', elite.length === 128, `${elite.length} laga league phase (2 zona x 16 tim x 8 laga / 2)`);
...
must('elite-32-klub-8-laga', eIds.length === 32 && eIds.every((id) => eHome[id] === 4 && eAway[id] === 4), `klub=${eIds.length}, tiap klub 4 home + 4 away`);
const eliteZones = aclEliteZones();
must('elite-16-per-zona', eliteZones.length === 2 && eliteZones.every((z) => z.ids.length === 16) && eliteZones[0].zone === 'east' && eliteZones[1].zone === 'west', eliteZones.map((z) => z.label + '=' + z.ids.length + ' tim').join(' | '));
...
must('elite-tanpa-ulangan', Object.values(ePairs).every((c) => c === 1), `${Object.keys(ePairs).length} pasangan (64/zona), tidak ada yang dobel`);
const eZone = {};
